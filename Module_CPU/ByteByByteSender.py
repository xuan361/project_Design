# python_sender_byte.py
# 从 machineCode.txt 读取指令（二进制格式），并逐字节发送
...
def read_machine_code(filename):
    ...
    with open(filename, 'r') as f:
        for line_num, line in enumerate(f, 1):
            ...
            # 验证是否是有效的16位二进制数
            if len(code) == 16 and all(c in '01' for c in code):
                instructions.append(code)
            else:
                print(f"警告: 在文件 '{filename}' 第 {line_num} 行忽略无效二进制码 -> '{line}'")
    
    return instructions

def main():
    machine_code_bin = read_machine_code(CODE_FILE) # 现在读取的是二进制字符串列表
    ...
    try:
        ...
        if num_instructions > 0:
            ...
            for i, instr_bin_str in enumerate(machine_code_bin): # instr_bin_str 是二进制字符串
                print(f"发送指令 {i+1}/{num_instructions}: {instr_bin_str}")
                # 将二进制字符串转换为整数
